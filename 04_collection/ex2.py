...
# 길이가 5 이상인 이름만 뽑기
result = [names[x] for x in range(0, len(names)) if len(names[x]) >= 5]
print(result)
...
result = [scores[x] for x in range(0, len(scores)) if scores[x] >= 60]
print(result)                       # ✅ [85, 73, 90, 68, 100] 출력


# 2️⃣ 60점 이상인 경우 "합격", 60점 미만은 "불합격"으로 처리
result = ["합격" if scores[x] >= 60 else "불합격" for x in range(0, len(scores))]
...
result = [words[x] for x in range(0, len(words)) if "n" in words[x]]  
